...
export default function checkout() {
  ...
  const popUp = () => {
    if(!email || !firstname || !lastname || !city || !province || !postal || !phone){
  ...
  if (popup) {
    popupElement = (
      <div className="fixed top-0 right-0 left-0 bottom-0 bg-gray-500 bg-opacity-50 flex justify-center items-center">
        <div className="bg-white p-5 rounded-md shadow-lg relative">
          ...
          <button className="text-center w-full" onClick={() => setPopup(false)}>Close</button>
        </div>
      </div>
    );
  }
